...
class GetAll:
    ...
    # 创建html文档,用于组织爬取的文件
    def creatHtml(self):
        with open(os.path.join(self.htmlsdirpath, self.htmlfile), "w",encoding='utf-8') as f:
            # 生成文档头
            message = """
            <!DOCTYPE html>
    ...
    def addMessageToHtml(self,message):
        ...
        with open(os.path.join(self.htmlsdirpath, self.htmlfile), "a",encoding='utf-8') as a:
            a.write(message)
    ...
    def convertHtmlToPdf(self):
        savepath = os.path.join(self.pdfsdirpath, self.wkinfo.get('title') + '.pdf')
        url = os.path.join('\pdffiles', self.wkinfo.get('title') + '.pdf')
        ...
        pdfkit.from_file(os.path.join(self.htmlsdirpath, self.htmlfile), savepath, options=options)
        return url

    def Run(self):
        if self.wkinfo.get('docType') == 'ppt':
            savePath = GetPpt(self.url, self.savepath).getPPT()
            htmlurl = "/htmlfiles/" + self.htmlfile
            flag = 1
            return savePath, flag , htmlurl

        if self.wkinfo.get('docType') == 'txt':
            savePath = GetTxt(self.url, self.savepath).getTXT()
            flag = 1
            htmlurl = "/htmlfiles/" + self.htmlfile
            return savePath, flag , htmlurl
...
# 文件预览
@app.route("/htmlfiles/<path:filename>")
def htmlView(filename):
    dirpath = os.path.join(app.root_path, "htmlfiles")  # 下载文件目录路径
    return send_from_directory(dirpath, filename)  # 打开html文件
...
def post():
    ...
    if request.method=='POST':
        ...
        url , flag , htmlurl = GetAll((request.form.get('url')),"").Run()
        # 直接用flash返回该文件地址，以及一个flag用作按钮显示
        flash(url)# 生成的pdf地址
        flash(flag)# 是否已生成文档，用来显示按钮
        flash(htmlurl)# html地址
